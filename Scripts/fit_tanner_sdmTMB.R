...
tanner.mod.df2 <- tanner_data$haul %>%
...
# # Plot
# ggplot(tanner.mod.df2 %>% filter(YEAR %in% 2016:2025), aes(LONGITUDE, LATITUDE, fill = ICE)) +
#   geom_tile(width = 45, height = 45) +
#   theme_bw() +
#   facet_wrap(~YEAR) +
#   scale_fill_viridis_c()
# 
# ggplot(tanner.mod.df2, aes(LONGITUDE, LATITUDE, fill = SED)) +
#   geom_tile(width = 45, height = 45) +
#   theme_bw() +
#   facet_wrap(~YEAR) +
#   scale_fill_viridis_c()
# 
ggplot(tanner.mod.df2 %>% filter(YEAR %in% 2021:2025), aes(LONGITUDE, LATITUDE, fill = DEPTH)) +
  geom_tile(width = 45, height = 45) +
  theme_bw() +
  facet_wrap(~YEAR) +
  scale_fill_viridis_c()

ggplot(tanner.mod.df2%>% filter(YEAR %in% 2021:2025), aes(LONGITUDE, LATITUDE, fill = log(CPUE+10))) +
  geom_tile(width = 45, height = 45) +
  theme_bw() +
  facet_wrap(~YEAR) +
  scale_fill_viridis_c()

ggplot(tanner.mod.df2 %>% filter(YEAR %in% 2021:2025), aes(LONGITUDE, LATITUDE, fill = BTEMP)) +
  geom_tile(width = 45, height = 45) +
  theme_bw() +
  facet_wrap(~YEAR) +
  scale_fill_viridis_c()
...
# Build mesh
mesh <- make_mesh(tanner.mod.df2, c("LONGITUDE","LATITUDE"), n_knots = 90, type = "kmeans")
...
# Resids
plot.resids(tanner.mod, "Tanner", "spRF")
