...
positive_catch <- hybrid_cpue %>% 
                    filter(CPUE >0) %>%
                    dplyr::select(STATION_ID, YEAR) %>% # this selects the unique year X station combinations for positive catch hybrids
                    distinct() 
...
out <- rbind(tanner_cpue, snow_cpue) %>% # bind snow and tanner cpue data frames
        ...
        filter(YEAR %in% c(2015:2025)) %>% # filter data to within the last ten years
          ...
          reframe(PROP_CPUE = SPECIES_CPUE/TOTAL_CPUE) # summarizing the data using reframe() to calculate proportion hybrid by size and sex accounting for total snow/tanner in the haul
...
# Analyze size distribution in recent year
